#!/bin/bash
# 改进接收端 v4：读路径也加熔断，避免 Redis 不可用时 /alerts 阻塞
set -u
B=/opt/order-stack/webhook
mkdir -p $B

cat > $B/receiver.py <<'PYEOF'
# -*- coding: utf-8 -*-
...
echo "  已写入 receiver.py（$(wc -l < $B/receiver.py) 行）"
python3 -c "import ast; ast.parse(open('$B/receiver.py').read()); print('  语法 OK')"
echo "RECEIVER-V4-WRITTEN"
